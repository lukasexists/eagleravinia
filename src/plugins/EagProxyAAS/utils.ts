...
export async function onConnect(client: ClientState) {
  try {
    ...
    sendCustomMessage(
      client.gameClient,
      "Select an option from the above (1 = online, 2 = offline - use this to join ravinia, 3 = EasyMC), either by clicking or manually typing out the option's number on the list.",
      "green"
    );
    ...
    if (chosenOption == ConnectType.ONLINE) {
      ...
      sendMessage(
        client.gameClient,
        `Provide a server to join or use /join ravinia. ${Enums.ChatColor.GOLD}/join <ip>${
          config.allowCustomPorts ? " [port]" : ""
        }${Enums.ChatColor.RESET}.`
      );
      let host: string, port: number;
