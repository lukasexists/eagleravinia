...
export function disconectIdle() {
    SERVER.players.forEach((client) => {
        if (client.state == ConnectionState.AUTH &&
        ...
        else if (client.state == ConnectionState.SUCCESS &&
...
export async function onConnect(client) {
    try {
        ...
        sendCustomMessage(client.gameClient, "Select an option from the above (1 = online, 2 = offline - use this to join ravinia, 3 = EasyMC), either by clicking or manually typing out the option's number on the list.", "green");
        ...
        if (chosenOption == ConnectType.ONLINE) {
            ...
            while (true) {
                ...
                if (parsed.length < 2)
                    ...
                    if (port != null && !config.allowCustomPorts) {
                        sendCustomMessage(client.gameClient, "You are not allowed to use custom server ports! /join <ip>" +
                            (config.allowCustomPorts ? " [port]" : ""), "red");
                        host = null;
                        port = null;
                    }
                    else {
                        if (host.match(/^(?:\*\.)?((?!hypixel\.net$)[^.]+\.)*hypixel\.net$/)) {
                            sendCustomMessage(client.gameClient, "Disallowed server, refusing to connect! Hypixel has been known to falsely flag Eaglercraft clients, and thus we do not allow connecting to their server. /join <ip>" +
                                (config.allowCustomPorts ? " [port]" : ""), "red");
                        }
                        else {
                            port = port ?? 25565;
            ...
            client.lastStatusUpdate = Date.now();
            updateState(client.gameClient, "SERVER");
            sendMessage(client.gameClient, `Provide a server to join or use /join ravinia. ${Enums.ChatColor.GOLD}/join <ip>${config.allowCustomPorts ? " [port]" : ""}${Enums.ChatColor.RESET}.`);
            let host, port;
            while (true) {
                const msg = await awaitCommand(client.gameClient, (msg) => msg.startsWith("/join")), parsed = msg.split(/ /gi, 3);
                if (parsed.length < 2)
                    ...
                    sendMessage(client.gameClient, `A valid port number has to be passed! ${Enums.ChatColor.GOLD}/join <ip>${config.allowCustomPorts ? " [port]" : ""}${Enums.ChatColor.RESET}.`);
                else if (parsed[1] == "ravinia") {
                    sendMessage(client.gameClient, `${Enums.ChatColor.GOLD}Joining Ravinia!`);
                    host = '142.44.206.140';
                    port = 25617;
                    break;
                }
        ...
        if (!client.gameClient.ended) {
            ...
            client.gameClient.end(Enums.ChatColor.YELLOW +
...
export function generateSpawnChunk() {
    ...
    chunk.setBlock(new Vec3(8, 67, 8), new McBlock(REGISTRY.blocksByName.sea_lantern.id, REGISTRY.biomesByName.the_end.id, 0));
